...
export class Renderer {
  constructor(mapRadius) {
    ...
    this.camera = {
      ...
      prevCenterX: 0,
      prevCenterY: 0,
      boundry: { left: 0, right: 0, top: 0, bottom: 0 },
    };
  ...
  determineIfObjectShouldBeRendered(objectToInspect) {
    if (
      objectToInspect.LocationX > this.camera.left &&
      objectToInspect.LocationX < this.camera.right &&
      objectToInspect.LocationY > this.camera.top &&
      objectToInspect.LocationY < this.camera.bottom
    ) {
      return true;
    } else {
      return false;
  ...
  renderMiniShip(ship, playerShipId) {
    ...
    } else if (ship.Type == "Charlie") {
      this.map.fillStyle = "yellow";
    } else if (ship.Type == "Delta") {
  ...
  renderShip(ship) {
    ...
    // this.map.translate(
    //   ship.LocationX * this.pixelsPerMeter - this.map.measureText(nameToDraw).width / 2,
    //   ship.LocationY * this.pixelsPerMeter + ship.Size * this.pixelsPerMeter * 1.5
    // );
  ...
  renderSound(sound) {
    if (Client.gameMode == "PLAY_MODE") {
      ...
      if (this.playerShip != null) {
        distanceFromPlayersShip =
          Math.sqrt(
            (this.playerShip.LocationX - sound.LocationX) * (this.playerShip.LocationX - sound.LocationX) +
              (this.playerShip.LocationY - sound.LocationY) * (this.playerShip.LocationY - sound.LocationY)
          ) / this.pixelsPerMeter;
      }
  ...
  renderEmail() {
    ...
    this.map.translate(
      this.availableWidth / 2 - this.map.measureText("EMAIL: davehardenbrook@yahoo.com").width / 2,
      210
    );
  ...
  renderName() {
    ...
    this.map.translate(
      this.availableWidth / 2 - this.map.measureText(textToRender).width / 2,
      this.availableHeight / 2 + 35
    );
  ...
  renderShieldButton() {
    ...
    this.map.translate(
      this.availableWidth - this.availablePixels * 0.25,
      this.availableHeight - this.availablePixels * 0.1
    );
  ...
  renderFireButton() {
    ...
    this.map.translate(
      this.availableWidth - this.availablePixels * 0.1,
      this.availableHeight - this.availablePixels * 0.1
    );
  ...
  renderDamgeIndicator() {
    ...
    this.map.translate(this.availableWidth * 0.9, this.availableHeight * 0.9);

    this.map.scale(totalLengthOfObject * this.pixelsPerMeter, totalLengthOfObject * this.pixelsPerMeter);

    if (this.playerShip.HullStrength / this.playerShip.MaxHullStrength <= 0.33) {
      this.map.strokeStyle = "rgba(255, 0, 0, 1.0)";
      this.map.fillStyle = "rgba(100, 0, 0, 1.0)";
    } else if (this.playerShip.HullStrength / this.playerShip.MaxHullStrength <= 0.66) {
    ...
    if (this.playerShip.PlasmaCannonStrength / this.playerShip.MaxPlasmaCannonStrength <= 0.33) {
      this.map.strokeStyle = "rgba(255, 0, 0, 1.0)";
      this.map.fillStyle = "rgba(100, 0, 0, 1.0)";
    } else if (this.playerShip.PlasmaCannonStrength / this.playerShip.MaxPlasmaCannonStrength <= 0.66) {
      this.map.strokeStyle = "rgba(255, 255, 0, 1.0)";
      this.map.fillStyle = "rgba(100, 100, 0, 1.0)";
    } else {
      this.map.strokeStyle = "rgba(0, 255, 0, 1.0)";
    ...
    if (this.playerShip.ThrusterStrength / this.playerShip.MaxThrusterStrength <= 0.33) {
      this.map.strokeStyle = "rgba(255, 0, 0, 1.0)";
      this.map.fillStyle = "rgba(100, 0, 0, 1.0)";
    } else if (this.playerShip.ThrusterStrength / this.playerShip.MaxThrusterStrength <= 0.66) {
      this.map.strokeStyle = "rgba(255, 255, 0, 1.0)";
      this.map.fillStyle = "rgba(100, 100, 0, 1.0)";
    } else {
      this.map.strokeStyle = "rgba(0, 255, 0, 1.0)";
